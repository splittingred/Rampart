...
if (empty($scriptProperties['data'])) return $modx->error->failure($modx->lexicon('rampart.ban_err_ns'));
$_DATA = $modx->fromJSON($scriptProperties['data']);
if (empty($_DATA)) return $modx->error->failure($modx->lexicon('rampart.ban_err_ns'));

if (empty($_DATA['id'])) {
    return $modx->error->failure($modx->lexicon('rampart.ban_err_ns'));
}
$ban = $modx->getObject('rptBan',$_DATA['id']);
if (empty($ban)) { return $modx->error->failure($modx->lexicon('rampart.ban_err_nf',array('id' => $_DATA['id']))); }
